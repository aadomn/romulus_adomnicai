/**
 * Romulus-N ARMv7-M implementation (w/ 1st-order masking countermeasure)
    ...
    const mask_key_uint32_t *ks)
{
    ...
    *clen = mlen + TAGBYTES;
    romulusn_init(state, state_m, tk1);
    romulusn_process_ad(
        state, state_m,
        (uint8_t *)ads, adlen,
        rtk_23, rtk_3m,
    ...
    romulusn_process_msg(
        ...
        tk1,
        ENCRYPT_MODE);
    romulusn_generate_tag((uint8_t *)cs + mlen, state, state_m);
    return 0;
}
    ...
    const mask_key_uint32_t *ks)
{
    ...
    *mlen = clen;
    ...
    romulusn_process_msg(
        ...
        tk1,
        DECRYPT_MODE);
    return romulusn_verify_tag((uint8_t *)cs + *mlen, state, state_m);
}
